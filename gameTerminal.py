...
print("##############################################################################")
print()
print("Welcome to CHASE, an interactive game for everyone!")
print()
game = Game()
game.draw_board()
...
while True:
    ...
    if choice == '3': # Exit game
        ...
        print("Exiting Game...")
        sys.exit(0)
    elif choice == '2': # Display 'Manual'
        print('\n')
        print("+++++++++++++++++ HOW TO PLAY +++++++++++++++++")
        print()
        print("CHASE can be played in two modes: the fugitive or the four agents.")
        print("After starting the game, you will have to select the mode you want")
        print("to play as.")
        print()
        print("All movements are diagonal only.")
        print("Commands in the gameplay are case sensitive, thus 'fr' is different from 'FR' or 'Fr'.")
        print()
        print(" ** FUGITIVE MOVES ** ")
        print("Fugitive can move in both forward direction as well as backward.")
        print("The fugitive's goal is to get to the bottom row without being caught")
        print("by the agents.")
        print()
        print(" ** AGENTS MOVES ** ")
        print("Agents can move in only forward direction.")
        print("The agent's goal is to trap the fugitive and prevent it from moving to")
...
print("############## Playing Game ##############")
print("NOTE: Moving to an invalid location on the board will mean you lose your chance and the opponent goes next!")
print()
print("CHASE has two modes, playing as the fugitive and playing as the agents.")
print("To play as the fugitive, press 'F' and Enter. To play as the agents, press 'A' and Enter")
print()
mode = input("Select your preferred mode: ")

while mode not in ('F', 'A', 'f', 'a'):
    print("Mode must either be 'F' or 'A' ONLY!!!")
    mode = input("Select your preferred mode: ")

print()

if mode in ('F', 'f'):
    # Playing as the fugitive and the AI plays as the agents
    print("############ You are playing as the fugitive! ############")
    print()
    while True:
        game.draw_board()
        game.move_fugitive()
        if game.fugitive_goal_reached():
            break
        game.agent_next_move()
        if game.caught_fugitive():
            break
        print()

else:
    print("############ You are playing as the agents! ############")
    print()
    # Playing as the agents and the AI plays as the fugitive
    while True:
        game.draw_board()
        game.move_agent()
        if game.caught_fugitive():
            break
        game.fugitive_next_move()
        if game.fugitive_goal_reached():
            break
        print()
        
        # print(game.fugitive_goal_reached())
        # print(game.fugitive)
        
        

# This fires when the fugitive is caught or when the fugitive wins
if game.caught_fugitive():
    game.draw_board()
    print(game.caught_fugitive())
    print()

if game.fugitive_goal_reached():
    game.draw_board()
    print("Fugitive has won!")
    print()
